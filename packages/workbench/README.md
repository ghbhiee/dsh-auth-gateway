...
**A turn's files preview inline, in its footer.** The workbench takes over the turn-tail chain — `conversation.chat.turnTail` is a single-winner seat that dsh's own ProducedFiles otherwise renders — and shows a chip per file a completed turn touched: the produced files from the `deliverables` signal, **plus** paths the assistant merely named in its prose that actually exist on disk under a read root. That prose scan is what catches a file a `bash` step wrote, which the structured signal alone misses; every candidate is `stat`-ed before it becomes a chip, so a mention of nothing real is dropped rather than shown as a dead link. Clicking a chip expands the file inline, through the same safe renderer the panel uses — a sandboxed HTML iframe (inert by default), an image, markdown, or a code block — not the host opener. Taking the seat over means dsh's own produced-file chips (which open through the host) are replaced by these.
...
- **Inline file mentions inside a message body still open through the host.** dsh renders some inline-code paths in a message as its own clickable mentions (the `chatFileMentions` service), and those open through the host opener. A plugin cannot re-point them at the workbench — cordis refuses a duplicate service registration (it fails plugin load, verified) and the mention buttons carry no stable hook to intercept, only compiled hash classes this plugin will not key on. The workbench's own inline preview is the **turn-footer chip row** it owns (see the design note), which does preview in place; the in-body mentions remain dsh's.
